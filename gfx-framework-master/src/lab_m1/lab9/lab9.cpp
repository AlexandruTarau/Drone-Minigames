...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
		...
		// Rotate the square around OY axis to face the camera's forward
		modelMatrix = glm::rotate(modelMatrix, atan2(forward.x, forward.z), glm::vec3(0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    int loc_mix_textures = glGetUniformLocation(shader->program, "mix_textures");
	if (texture1 && texture2) {
		glUniform1f(loc_mix_textures, true);
    }
    else {
        glUniform1f(loc_mix_textures, false);
    }
    ...
    if (texture1)
    {
		...
		int loc_texture = glGetUniformLocation(shader->program, "texture_1");
    ...
    if (texture2)
    {
		...
		int loc_texture = glGetUniformLocation(shader->program, "texture_2");
