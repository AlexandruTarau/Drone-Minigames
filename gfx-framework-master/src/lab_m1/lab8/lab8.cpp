...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int cut_off_angle1_location = glGetUniformLocation(shader->program, "cut_off_angle1");
	glUniform1f(cut_off_angle1_location, cut_off_angle1);

    int cut_off_angle2_location = glGetUniformLocation(shader->program, "cut_off_angle2");
    glUniform1f(cut_off_angle2_location, cut_off_angle2);
    ...
    int light_type_location = glGetUniformLocation(shader->program, "light_type1");
    glUniform1i(light_type_location, light_type1);

    int light_type_location2 = glGetUniformLocation(shader->program, "light_type2");
    glUniform1i(light_type_location2, light_type2);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
		...
		if (window->KeyHold(GLFW_KEY_N)) cut_off_angle1 += 1;
        if (window->KeyHold(GLFW_KEY_J)) cut_off_angle1 -= 1;

        // TODO(student): Set any other keys that you might need
        if (window->KeyHold(GLFW_KEY_UP)) lightPosition2 -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_LEFT)) lightPosition2 -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_DOWN)) lightPosition2 += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_RIGHT)) lightPosition2 += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_O)) lightPosition2 += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_P)) lightPosition2 -= up * deltaTime * speed;

		if (window->KeyHold(GLFW_KEY_M)) cut_off_angle2 += 1;
		if (window->KeyHold(GLFW_KEY_K)) cut_off_angle2 -= 1;
    }
}
...
void Lab8::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_F) {
        // Toggle to spotlight
		light_type1 = (light_type1 + 1) % 2;
	}
	if (key == GLFW_KEY_G) {
		// Toggle to directional light
		light_type2 = (light_type2 + 1) % 2;
	}
}
